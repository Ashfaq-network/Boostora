...
export default async function handler(req) {
  ...
  try {
    ...
    const crawlTimeout = AbortSignal.timeout(3000);

    // PSI fetch with retry
    async function fetchPSI() {
      const PSI_URL = `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(url)}&strategy=mobile&key=${PSI_KEY}`;
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          const res = await fetch(PSI_URL, { signal: AbortSignal.timeout(8000) });
          if (!res.ok) { console.warn('PSI HTTP error:', res.status); return; }
          const mob    = await res.json();
          const cats   = mob?.lighthouseResult?.categories;
          const audits = mob?.lighthouseResult?.audits;
          if (!cats) { console.warn('PSI: no categories in response'); return; }
          ...
          console.log(`PSI success (attempt ${attempt}) — perf:`, psiData.performance, 'seo:', psiData.seo);
          return;
        } catch(e) {
          console.warn(`PSI attempt ${attempt} failed:`, e.message);
    ...
    await Promise.allSettled([
      PSI_KEY ? fetchPSI() : Promise.resolve(),
      fetch(url, {
