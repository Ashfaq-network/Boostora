...
export default async function handler(req) {
  ...
  try {
    ...
    const psiTimeout   = AbortSignal.timeout(7000);
    const crawlTimeout = AbortSignal.timeout(3000);

    await Promise.allSettled([
      // PageSpeed mobile only (saves ~3-4s vs fetching desktop too)
      PSI_KEY ? fetch(
        `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(url)}&strategy=mobile&category=performance&category=seo&category=accessibility&category=best-practices&fields=lighthouseResult(categories,audits(first-contentful-paint,largest-contentful-paint,total-blocking-time,cumulative-layout-shift,speed-index))&key=${PSI_KEY}`,
        { signal: psiTimeout }
      ).then(async r => {
        if (!r.ok) return;
