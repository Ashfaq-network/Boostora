...
export default async function handler(req) {
  ...
  try {
    ...
    const psiTimeout   = AbortSignal.timeout(7500);
    const crawlTimeout = AbortSignal.timeout(3000);

    await Promise.allSettled([
      // PageSpeed mobile only — clean URL, no fields filter
      PSI_KEY ? fetch(
        `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(url)}&strategy=mobile&key=${PSI_KEY}`,
        { signal: psiTimeout }
      ).then(async res => {
        ...
        console.log('PSI cats:', cats ? Object.keys(cats) : 'null');
        if (!cats) return;
        ...
        console.log('PSI success — perf:', psiData.performance, 'seo:', psiData.seo);
